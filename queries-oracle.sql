...
t2 as (
select 'Fl�klypa' adresse from dual
)
...
-- Join gotcha
-- Filtrering (where) etter left join kan føre til en inner join
-- For å unngå dette, filtrer før join gjøres (join on t1.x=t2.y AND t2.col='foo')
-- Se https://stackoverflow.com/questions/1018952/condition-within-join-or-where (2nd upvoted answer)
-- https://stackoverflow.com/questions/4752455/left-join-with-where-clause
-- Eksempel: Alle rader fra t1 skal inkluderes, og ev. rader fra t2 dersom t2.somecol='A'
-- For å oppnå dette må det filtreres på t2 før join
with t1 as (
    select 1 as pk, 'foo1' as col1 from dual union all
    select 2, 'foo2' from dual union all
    select 3, 'foo3' from dual),
t2 as (
    select 1 as fk, 'A' as somecol from dual union all
    select 3, 'B' from dual)
select * from t1 left join t2 on t1.pk = t2.fk
and t2.somecol='A' -- OK, filtrering før join, her blir alle kolonner fra t1 inkludert
--where (t2.somecol = 'A' or t2.somecol is null) -- t2.somecol='B' ved t2.fk=3, og oppfyller ikke dette kriteriet!
order by t1.pk;
...
-- Kjøre SQL fra Unix
--sqlplus -s $USER/$PASS@$HOST @$PROCEDURE > foo.csv
